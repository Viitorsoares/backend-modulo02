        ...
        if((requisicao.method === "PATCH" || requisicao.method === "PUT") & requisicao.is('application/json')) {
            ...
            if (codigo && codigo > 0 && titulo && data_evento && valor_ingreco && endereco && descricao){
                ...
                .catch((erro) => {
                    ...
                    resposta.json({
                        "status": false,
                        "mensagem": "Não foi possivel encontrar o cliente!" + erro.message
                    })
                })
            } else {
                resposta.status(400),
            ...
            resposta.json({
                "status":false,
                "mensagem": "Requisição inválida! Esperando o método PATCH ou PUT e dados no formato JSON para gravar um evento! "
            })
        }
    }
    ...
    excluir(requisicao, resposta) {
        ...
        if (requisicao.method === "DELETE") {
            ...
            if (codigo && codigo > 0) {
                ...
                resposta.json({
                    "status":false,
                    "mensagem": "Por favor, informe o código do cliente que deseja excluir, conforme a documentação da API!"
                })
            }
        } else {
            resposta.status(405);
    ...
    consultar(requisicao, resposta) {
        ...
        if (requisicao.method === "GET") {
            ...
            .catch((erro) => {
                ...
                resposta.json({
                    "status":false,
                    "mensagem": "Não foi possivel consultar os clientes!" + erro.message
                })
            })
        } else {
            resposta.status(405);
            resposta.json({
                "status":false,
                "mensagem": "Requisição inválida! Esperando o método GET para consultar os cliente!"
            })
        }
    }
}